...
def BestApproximate(n):
    
    C = []
    for i in range(2*n+1):
        y, err = integrate.quad(lambda x : x**i,0,math.pi)
        C.append(y)
    ...
    b = []
    for i in range(n+1):
        y, err = integrate.quad(lambda x : x**i*(math.sin(x)+math.cos(x)),0,math.pi)
        b.append(y)

    cof = np.linalg.solve(np.array(A),np.array(b))
    ...
    Y = np.sin(X)+np.cos(X) 
    plt.scatter(X,Y,marker ='+',color = 'r',label = "sin(x)+cos(x)")
    plt.title("Polynomial of degree {}".format(n))
    plt.legend()
    plt.show()
    return p
