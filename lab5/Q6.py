...
def OrthogonalityDemo():
    ChebyshevPolynomials = []
    for i in range(5):
        ChebyshevPolynomials.append(NthChebyshevPolynomial(i))
        #print(ChebyshevPolynomials[i])
    for i in range(5):
        for j in range(5):
            p = ChebyshevPolynomials[i]
            q = ChebyshevPolynomials[j]
            fn = p * q
            y, err = integrate.quad(lambda x : fn[x]/math.sqrt(1-x**2),-1,1)
            print(round(y,1),end=' ')
        print()
    print('All non diagonal integrals are zero implies orthogonality')
